...
export const StatutoryComplaintBanner: React.FC<StatutoryComplaintBannerProps> = ({
  ...
  return (
    <div
      className={`relative overflow-hidden rounded-3xl border border-rose-300 dark:border-rose-900/70 bg-gradient-to-br from-rose-50 via-white to-red-50/50 dark:from-rose-950/40 dark:via-slate-900 dark:to-slate-900/90 shadow-lg shadow-rose-500/10 p-4 sm:p-6 transition-all ${className}`}
    >
      ...
      <div className="relative z-10 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4 sm:gap-5">
        
        {/* Left: Defect Alert Information & Statutory Ramifications */}
        <div className="space-y-2 flex-1 min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <span className="inline-flex items-center gap-1.5 px-2.5 sm:px-3 py-1 rounded-full text-[11px] sm:text-xs font-black uppercase tracking-wider bg-rose-600 text-white shadow-xs shadow-rose-600/30 animate-pulse">
              <AlertOctagon className="w-3.5 h-3.5" />
              <span>{violationCount} Statutory Breaches</span>
            </span>

            <span className="inline-flex items-center gap-1 text-[10px] sm:text-[11px] font-bold px-2 py-0.5 rounded-full bg-rose-100 dark:bg-rose-900/50 text-rose-800 dark:text-rose-200 border border-rose-200 dark:border-rose-800/60">
              <Scale className="w-3 h-3" />
              <span>Sec 36 LMPC & FSSA</span>
            </span>
          </div>

          <div>
            <h3 className="text-base sm:text-lg font-black text-slate-900 dark:text-white tracking-tight leading-snug">
              File Statutory Complaint to Food Safety Desks
            </h3>
            <p className="text-xs sm:text-sm text-slate-600 dark:text-slate-300 mt-1 leading-relaxed">
              This product (<span className="font-bold text-slate-900 dark:text-white">{report.brand_name} - {report.product_name}</span>) violates mandatory labelling requirements. Transmit a legally grounded petition in just 1 click.
            </p>
          </div>
          ...
          <div className="pt-1 flex flex-wrap items-center gap-2 text-xs">
            <span className="text-slate-500 dark:text-slate-400 font-semibold flex items-center gap-1 shrink-0">
              <Building2 className="w-3.5 h-3.5 text-rose-600 dark:text-rose-400" /> Target Desk:
            </span>
            <select
              value={selectedDeskKey}
              onChange={(e) => setSelectedDeskKey(e.target.value)}
              className="text-xs font-bold py-1 px-2.5 rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-200 focus:ring-2 focus:ring-rose-500 outline-none shadow-2xs max-w-full"
            >
        ...
        <div className="flex flex-col sm:flex-row lg:flex-col items-stretch gap-2.5 w-full lg:w-auto shrink-0 mt-1 lg:mt-0">
          ...
          <div className="grid grid-cols-2 gap-2 w-full">
            ...
            <button
              ...
              className="px-2.5 sm:px-3 py-2 rounded-xl bg-white dark:bg-slate-800 hover:bg-rose-50 dark:hover:bg-rose-950/40 border border-rose-200 dark:border-rose-900/60 text-slate-800 dark:text-slate-200 font-bold text-[11px] sm:text-xs flex items-center justify-center gap-1.5 shadow-2xs transition active:scale-95 cursor-pointer tap-transparent"
              title={`Send pre-filled statutory notice directly to ${activeDesk.primaryEmail}`}
            >
              <Send className="w-3.5 h-3.5 text-rose-600 dark:text-rose-400 shrink-0" />
              <span className="truncate">Email Dept</span>
            </button>

            {/* 1-Click Copy Petition */}
            <button
              onClick={handleCopyLegalText}
              className="px-2.5 sm:px-3 py-2 rounded-xl bg-white dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-slate-200 font-bold text-[11px] sm:text-xs flex items-center justify-center gap-1.5 shadow-2xs transition active:scale-95 cursor-pointer tap-transparent"
            ...
            >
              {copied ? (
                ...
                <>
                  ...
                  <span className="truncate">Copy Petition</span>
                </>
              )}
            </button>
      ...
      <div className="mt-3.5 pt-3 border-t border-rose-200/60 dark:border-rose-900/40 flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs">
        <div className="flex items-center gap-1.5 text-[10px] sm:text-[11px] font-bold text-slate-500 dark:text-slate-400">
          <ShieldAlert className="w-3.5 h-3.5 text-rose-500 shrink-0" />
          <span>Official 1-Click Portals:</span>
        </div>
        ...
        <div className="flex overflow-x-auto no-scrollbar items-center gap-1.5 py-0.5">
          {OFFICIAL_GOVT_PORTALS.map(portal => (
            <button
              ...
              className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-[10px] sm:text-[11px] font-bold bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-200 hover:border-brand-500 hover:text-brand-600 dark:hover:text-brand-400 transition shadow-2xs cursor-pointer active:scale-95 shrink-0 tap-transparent"
