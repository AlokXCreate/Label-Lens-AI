  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden bg-slate-900/60 backdrop-blur-xs flex justify-end animate-in fade-in">
      <div className="w-full max-w-2xl bg-white h-full shadow-2xl flex flex-col border-l border-slate-200">
        ...
        {/* Dossier Content */}
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-5 text-xs sm:text-sm text-slate-800 font-sans">
          
          {/* Multi-Format Download Options Bar */}
          <div className="bg-gradient-to-br from-slate-50 to-indigo-50/40 p-4 rounded-2xl border border-indigo-100 shadow-xs">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-5 gap-2">
              ...
              <button
                ...
                className="flex flex-col items-center justify-center p-2.5 rounded-xl border border-rose-200 bg-white hover:bg-rose-50 hover:border-rose-300 text-slate-700 transition active:scale-95 group shadow-xs"
                title="Download Print-Ready Legal Petition PDF"
              >
                <FileText className="w-4 h-4 text-rose-600 group-hover:scale-110 transition" />
                <span className="text-xs font-bold mt-1 text-slate-900">{t('downloadPdf')}</span>
              ...
              <button
                ...
                className="flex flex-col items-center justify-center p-2.5 rounded-xl border border-sky-200 bg-white hover:bg-sky-50 hover:border-sky-300 text-slate-700 transition active:scale-95 group shadow-xs disabled:opacity-50"
              ...
              >
                ...
                <span className="text-xs font-bold mt-1 text-slate-900">{t('downloadWord')}</span>
              ...
              <button
                ...
                className="flex flex-col items-center justify-center p-2.5 rounded-xl border border-brand-300 bg-brand-50/70 hover:bg-brand-100 text-brand-900 transition active:scale-95 group shadow-xs"
                title="Download Branded Application with Logo (.html)"
              >
                <Globe className="w-4 h-4 text-brand-600 group-hover:scale-110 transition" />
                <span className="text-xs font-bold mt-1 text-brand-900">{t('downloadHtml')}</span>
                <span className="text-[9px] text-brand-600 font-semibold">With Logo</span>
              </button>

              {/* JSON Data */}
              <button
                onClick={() => handleDownload('json')}
                className="flex flex-col items-center justify-center p-2.5 rounded-xl border border-amber-200 bg-white hover:bg-amber-50 hover:border-amber-300 text-slate-700 transition active:scale-95 group shadow-xs"
                title="Download Structured JSON Schema"
              >
                <Code className="w-4 h-4 text-amber-600 group-hover:scale-110 transition" />
                <span className="text-xs font-bold mt-1 text-slate-900">{t('downloadJson')}</span>
              ...
              <button
                ...
                className="col-span-2 sm:col-span-1 flex flex-col items-center justify-center p-2.5 rounded-xl border border-slate-200 bg-white hover:bg-slate-100 text-slate-700 transition active:scale-95 group shadow-xs"
                title="Download Plain Text for Grievance Forms"
              >
                <FileText className="w-4 h-4 text-slate-600 group-hover:scale-110 transition" />
                <span className="text-xs font-bold mt-1 text-slate-900">{t('downloadTxt')}</span>
          ...
          <div className="p-3.5 bg-rose-50/70 rounded-xl border border-rose-200">
            ...
            <div className="font-bold text-rose-950 mt-0.5 leading-snug">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div className="p-3.5 bg-slate-50 rounded-xl border border-slate-200">
              ...
              <div className="font-bold text-slate-900 mt-1">{complaint.complainant.name}</div>
              <div className="text-slate-600 text-xs mt-0.5">{complaint.complainant.email}</div>
              <div className="text-slate-600 text-xs">{complaint.complainant.phone}</div>
              <div className="text-slate-500 text-[11px] mt-1 line-clamp-2">{complaint.complainant.address}</div>
            </div>
            ...
            <div className="p-3.5 bg-slate-50 rounded-xl border border-slate-200">
              ...
              <div className="font-bold text-slate-900 mt-1">{complaint.respondent.brand_name}</div>
              <div className="text-slate-600 text-xs mt-0.5">{complaint.respondent.manufacturer_name}</div>
              <div className="text-slate-500 text-[11px] mt-1 line-clamp-2">{complaint.respondent.premises_address}</div>
              {complaint.respondent.fssai_license_number && (
                <div className="text-emerald-700 font-mono font-bold text-[11px] mt-1">
          ...
          {complaint.incident_location && (
            <div className="p-3.5 bg-sky-50 rounded-xl border border-sky-200 flex items-start gap-3">
              ...
              <div className="text-xs flex-1">
                <span className="font-bold text-sky-950 block">Geo-Tagged Location Attached:</span>
                <span className="text-sky-800">{complaint.incident_location.formatted_address}</span>
                <div className="flex items-center gap-3 mt-1.5">
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="font-bold text-sky-700 hover:text-sky-900 underline inline-flex items-center gap-1"
                  >
              ...
              {complaint.itemized_violations.map((v, i) => (
                <div key={i} className="p-3.5 rounded-xl bg-slate-50 border border-slate-200 text-xs">
                  <div className="flex items-start justify-between gap-2">
                    <span className="font-bold text-slate-900">{i + 1}. {v.rule_title}</span>
                    <span className="font-mono text-[11px] text-rose-700 font-bold bg-rose-50 px-1.5 py-0.5 rounded border border-rose-200 shrink-0">
                      {v.violated_statute}
                    </span>
                  </div>
                  <div className="text-slate-700 mt-1.5 leading-relaxed">
                    <strong className="text-slate-900">Finding:</strong> {v.observed_finding}
                  </div>
                  <div className="text-rose-700 mt-1 leading-relaxed bg-white p-2 rounded-lg border border-rose-100">
          ...
          <div>
            ...
            <div className="bg-slate-50 border border-slate-200 rounded-xl p-3.5">
              <ul className="space-y-2 list-none text-xs text-slate-700 leading-relaxed">
                {complaint.formal_prayer.map((p, i) => (
                  <li key={i} className="flex items-start gap-2">
                    <span className="font-bold text-brand-700 shrink-0">({String.fromCharCode(97 + i)})</span>
                    <span>{p}</span>
                  </li>
                ))}
              </ul>
          ...
          <div className="p-3 bg-emerald-50/70 border border-emerald-200 rounded-xl flex items-center justify-between text-xs">
            <div className="flex items-center gap-2">
              ...
              <span className="font-bold text-emerald-950">Cryptographic Integrity Stamp:</span>
            </div>
        ...
        <div className="p-4 border-t border-slate-200 bg-slate-50 flex flex-wrap items-center justify-between gap-3">
          <button
            onClick={handleCopy}
            className="flex items-center gap-1.5 px-3.5 py-2 text-xs font-semibold rounded-xl bg-white border border-slate-300 text-slate-700 hover:bg-slate-100 transition shadow-xs active:scale-95"
          >
