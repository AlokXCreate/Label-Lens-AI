...
export const MobileAppView: React.FC<MobileAppViewProps> = ({
  ...
  return (
    <div className="flex flex-col min-h-full bg-slate-50 text-slate-900 dark:bg-slate-950 dark:text-slate-100 transition-colors duration-200">
      ...
      {/* Mobile Top App Bar */}
      <header className="sticky top-0 z-30 bg-white/95 dark:bg-slate-900/95 text-slate-900 dark:text-white backdrop-blur-md px-3 sm:px-4 py-2.5 pt-safe border-b border-slate-200 dark:border-slate-800 shadow-xs flex items-center justify-between transition-colors duration-200">
        <div className="flex items-center gap-2 min-w-0">
          <LabelLensLogo className="w-7 h-7 shrink-0" />
          <div className="min-w-0">
            <div className="flex items-center gap-1.5">
              <span className="font-black text-sm tracking-tight text-slate-900 dark:text-white truncate">Label Lens AI</span>
              <span className="px-1.5 py-0.2 bg-emerald-500/20 text-emerald-700 dark:text-emerald-300 font-mono font-bold text-[9px] rounded border border-emerald-500/30 shrink-0">
                APK
              </span>
            </div>
            <p className="text-[10px] text-slate-500 dark:text-slate-400 -mt-0.5 truncate max-w-[130px] sm:max-w-[200px]">
              {t('appSubtitle')}
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          {/* Language Switcher Pill (Compact Flag + Code) */}
          <button
            onClick={onOpenLanguageSelector}
            className="flex items-center gap-1 px-2 py-1 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 border border-slate-200 dark:border-slate-700 transition active:scale-95 shadow-xs tap-transparent"
            title="Switch Language"
          >
            <span className="text-xs">{currentLanguageMeta.flag}</span>
            <span className="text-[10px] font-extrabold uppercase tracking-wider">{currentLanguageMeta.code.toUpperCase()}</span>
          </button>

          {/* Theme Toggle Button */}
          <button
            onClick={toggleTheme}
            className="p-1.5 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 border border-slate-200 dark:border-slate-700 transition active:scale-95 shadow-xs tap-transparent"
          ...
          <button
            ...
            className="p-1.5 rounded-xl bg-brand-600 hover:bg-brand-500 text-white shadow-xs transition active:scale-95 tap-transparent"
          ...
          <button
            ...
            className="p-1.5 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-700 transition active:scale-95 shadow-xs tap-transparent"
            title="Settings"
          >
      ...
      <main className="flex-1 p-3 sm:p-4 space-y-4 pb-28">
            ...
            {currentReport && (
              currentReport.compliance_score < 90 || currentReport.findings.some(f => f.status === 'Non-Compliant') ? (
                <div className="p-4 bg-gradient-to-r from-rose-950/90 via-slate-900 to-slate-900 text-white rounded-3xl border border-rose-500/50 shadow-lg shadow-rose-950/40 flex flex-col gap-3">
                  ...
                  <div className="flex items-center gap-2 pt-1">
                    <button
                      ...
                      className="flex-1 py-2.5 px-3 rounded-xl bg-gradient-to-r from-rose-600 to-red-600 hover:from-rose-700 hover:to-red-700 text-white font-black text-xs flex items-center justify-center gap-1.5 shadow-md shadow-rose-600/30 active:scale-95 transition cursor-pointer tap-transparent"
                    >
                    ...
                    <button
                      ...
                      className="py-2.5 px-3 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 font-bold text-xs flex items-center justify-center gap-1 active:scale-95 transition tap-transparent"
                    >
                      <span>Findings</span>
                ...
                <div className="p-3.5 bg-gradient-to-r from-emerald-950/80 via-slate-900 to-slate-900 text-white rounded-2xl border border-emerald-500/50 shadow-md flex items-center justify-between">
                  ...
                  <button
                    onClick={() => setActiveTab('audit')}
                    className="px-3 py-1.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs flex items-center gap-1 shadow-sm transition active:scale-95 shrink-0 tap-transparent"
                  >
          ...
          <div className="space-y-4 animate-in fade-in duration-200">
            {currentReport ? (
              ...
              <div className="p-8 text-center bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm space-y-3">
                ...
                <h3 className="font-bold text-slate-800 dark:text-slate-200 text-sm">No Active Audit Dossier</h3>
                ...
                <button
                  ...
                  className="px-4 py-2 bg-brand-600 hover:bg-brand-700 text-white text-xs font-bold rounded-xl shadow-md transition tap-transparent"
                >
                  Start Scanning
                </button>
              </div>
          ...
          <div className="space-y-3 animate-in fade-in duration-200">
            <div className="p-4 bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm transition-colors duration-200">
              ...
              <button
                ...
                className="w-full py-3 rounded-xl bg-brand-600 hover:bg-brand-700 text-white font-bold text-xs shadow-md shadow-brand-500/25 flex items-center justify-center gap-2 transition active:scale-95 tap-transparent"
              >
            ...
            <div className="space-y-2">
              <div className="p-3 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 text-xs transition-colors duration-200">
                <div className="font-bold text-slate-900 dark:text-white">Controller of Legal Metrology, Maharashtra</div>
                <div className="text-slate-500 dark:text-slate-400 text-[11px] mt-0.5">Barrack No. 7, Free Press Journal Marg, Nariman Point, Mumbai</div>
                <div className="text-brand-600 dark:text-brand-400 font-bold text-[11px] mt-1">Tel: 022-22886666</div>
              </div>
              <div className="p-3 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 text-xs transition-colors duration-200">
                <div className="font-bold text-slate-900 dark:text-white">FSSAI Western Regional Office</div>
                <div className="text-slate-500 dark:text-slate-400 text-[11px] mt-0.5">902, Subharambh Building, Mumbai 400051</div>
                <div className="text-brand-600 dark:text-brand-400 font-bold text-[11px] mt-1">Tel: 022-26591000</div>
              </div>
            </div>
          </div>
        )}
        ...
        {activeTab === 'helpline' && (
          <div className="space-y-3 animate-in fade-in duration-200">
            <div className="p-4 bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm space-y-3 transition-colors duration-200">
              ...
              <a
                href="tel:1800112100"
                className="w-full py-3 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-black text-xs shadow-md shadow-emerald-600/25 flex items-center justify-center gap-2 transition active:scale-95 tap-transparent"
              >
                <PhoneCall className="w-4 h-4" />
                <span>Call National Consumer Helpline (1915)</span>
              </a>

              {/* In-App Calling Desk & Log Recorder */}
              <button
                onClick={onOpenCallModal}
                className="w-full py-2.5 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 font-bold text-xs flex items-center justify-center gap-2 transition active:scale-95 tap-transparent border border-slate-200 dark:border-slate-700"
              >
          ...
          <div className="space-y-3 animate-in fade-in duration-200">
            ...
            <div className="p-4 bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm transition-colors duration-200">
              <div className="flex items-center gap-3">
                <div className="w-12 h-12 rounded-2xl bg-gradient-to-tr from-cyan-600 via-indigo-600 to-emerald-500 text-white flex items-center justify-center font-black text-lg shadow-md shadow-brand-500/20 shrink-0">
