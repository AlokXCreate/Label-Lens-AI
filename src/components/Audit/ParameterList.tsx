        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="text-xs pl-8 pr-3 py-1.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 focus:bg-white dark:focus:bg-slate-900 focus:ring-2 focus:ring-brand-500 outline-none w-36 sm:w-44 transition"
            />
          </div>

          <div className="flex bg-slate-100 dark:bg-slate-800 p-1 rounded-xl text-xs font-bold text-slate-600 dark:text-slate-400 border border-slate-200/60 dark:border-slate-700">
            <button
              onClick={() => setFilter('ALL')}
              className={`px-3 py-1 rounded-lg transition ${filter === 'ALL' ? 'bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100 shadow-xs' : 'hover:text-slate-900 dark:hover:text-slate-200'}`}
            >
              All ({findings.length})
            </button>
            <button
              onClick={() => setFilter('NON_COMPLIANT')}
              className={`px-3 py-1 rounded-lg transition ${filter === 'NON_COMPLIANT' ? 'bg-white dark:bg-slate-700 text-rose-600 dark:text-rose-400 shadow-xs' : 'hover:text-rose-600 dark:hover:text-rose-400'}`}
            >
              Violations ({violationCount})
            </button>
            <button
              onClick={() => setFilter('COMPLIANT')}
              className={`px-3 py-1 rounded-lg transition ${filter === 'COMPLIANT' ? 'bg-white dark:bg-slate-700 text-emerald-600 dark:text-emerald-400 shadow-xs' : 'hover:text-emerald-600 dark:hover:text-emerald-400'}`}
            >
      ...
      <div className="divide-y divide-slate-100 dark:divide-slate-800">
        {filteredFindings.map((item, idx) => {
          ...
          return (
            <div
              key={idx}
              className={`p-4 sm:p-5 transition hover:bg-slate-50/75 dark:hover:bg-slate-800/40 ${
